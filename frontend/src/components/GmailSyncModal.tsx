...
import { gmailSyncService, GmailSyncStatus } from '../services/gmailSync';
...
const GmailSyncModal: React.FC<GmailSyncModalProps> = ({
  isOpen,
  onSyncComplete
}) => {
  ...
  useEffect(() => {
    ...
    const initializeSync = async () => {
      try {
        const status = await gmailSyncService.getSyncStatus();
        setSyncStatus(status);
        
        // Auto-start sync if needed
        if (status.needed) {
          await gmailSyncService.startSync();
        }
      } catch (error) {
